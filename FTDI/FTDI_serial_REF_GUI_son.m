function varargout = FTDI_serial_REF_GUI_son(varargin)
%ati' nin malidir.......
% Last Modified by GUIDE v2.5 30-Sep-2013 12:48:48
%%
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @FTDI_serial_REF_GUI_son_OpeningFcn, ...
                   'gui_OutputFcn',  @FTDI_serial_REF_GUI_son_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
%%
% --- Executes just before FTDI_serial_REF_GUI_son is made visible.
function FTDI_serial_REF_GUI_son_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to FTDI_serial_REF_GUI_son (see VARARGIN)
handles.degisken=1;
...
handles.FTDI_Assembly= NET.addAssembly('D:\DropBox_Santez\Dropbox\MATLAB_ati_ozel\GUI(my)\SMART_GRID\HaziladigimYazilim-GUIler\XbeePaketHaberlesme\NET_serials_kisim\FTD2XX_NET.dll');
...
% Choose default command line output for FTDI_serial_REF_GUI_son
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);
% UIWAIT makes FTDI_serial_REF_GUI_son wait for user response (see UIRESUME)
...
function varargout = FTDI_serial_REF_GUI_son_OutputFcn(hObject, eventdata, handles) 
...
function pushbutton4_Callback(hObject, eventdata, handles)
              TxQueue=uint32(0);
               num_tx=uint32(0);
                                   Sending_Cirrus_Data_Array_uint8 = uint8([get(handles.edit2,'String') get(handles.edit3,'String') get(handles.edit4,'String')]);
                                   Tx_Data_length_int32=int32(length(Sending_Cirrus_Data_Array_uint8));
                                   [status_Tx_Available,TxQueue]=GetTxBytesWaiting(handles.PortOBJ,TxQueue);
                                   if ((status_Tx_Available==handles.FT_OK)&&(TxQueue==0))
                                              [status_Write,num_tx]=Write(handles.PortOBJ, Sending_Cirrus_Data_Array_uint8,Tx_Data_length_int32,num_tx);
                                   end
